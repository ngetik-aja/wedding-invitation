...
func (r *CustomerRepository) FindByEmail(ctx context.Context, email string) (model.Customer, bool, error) {
	...
	row := r.DB.QueryRow(ctx, `
		SELECT id, full_name, email, password_hash, domain, status, created_at
		FROM customers
		WHERE email = $1
		LIMIT 1
	`, email)
	if err := row.Scan(&customer.ID, &customer.FullName, &customer.Email, &customer.PasswordHash, &customer.Domain, &customer.Status, &customer.CreatedAt); err != nil {
